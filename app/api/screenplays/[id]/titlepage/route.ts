...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (writtenDate !== undefined) {
      data.writtenDate = writtenDate && typeof writtenDate === 'string' ? new Date(writtenDate) : null;
    }
    ...
    const msg = error instanceof Error ? error.message : 'Failed to update title page';
