  ...
  return (
    <div className="max-w-[1200px] mx-auto px-6 md:px-12 py-10">
      ...
      <div className="grid grid-cols-3 gap-4 mb-10">
        <StatCard label="Total Screenplays" value={screenplays.length} />
        <StatCard label="Active Collaborations" value={screenplays.reduce((sum, s) => sum + s._count.collaborators, 0)} />
        <StatCard label="Film / Series" value={`${screenplays.filter(s => s.type === 'FILM').length} / ${screenplays.filter(s => s.type === 'SERIES').length}`} />
      </div>

      {/* Screenplays */}
      {screenplays.length === 0 ? (
        <div className="text-center py-20">
          <h2 className="text-xl font-semibold text-txt-primary mb-2">Every masterpiece starts with a single line.</h2>
          <p className="text-txt-secondary mb-6">Create your first screenplay to get started.</p>
          <Link href="/screenplay/new">
            <Button variant="primary" size="lg">Write Your First Screenplay</Button>
          </Link>
        </div>
      ) : (
        <>
